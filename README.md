...
- **⏱️ 进度条时间显示优化**：修复AI对话进度条计时不符合实际工作时间的问题
  ...
  - 将 ETA（预计剩余时间）改为显示已用时间（⏱️），更直观反映实际工作时长
  - 时间实时递增，与实际AI思考时间一致
...
- **⏱️ 进度条时间计数修复**：修复进度条 ETA 计算时间不准确的问题
  ...
  - 避免进度条创建后等待期间的时间被计入，导致 ETA 计算不准确
  - 确保时间计数与实际思考时间一致
...
- **🔄 列重命名数据迁移**：添加列重命名数据迁移逻辑，避免数据丢失
  - 支持 `applied` → `is_applied`、`preference_key` → `config_key` 等列重命名
  - 自动检测旧列是否存在并复制数据到新列

- **🔄 主键类型迁移**：为主键类型不匹配的表添加表重建迁移逻辑
  - 将 `self_update_history`、`confirmation_history` 等表的主键从 `INTEGER AUTOINCREMENT` 改为 `TEXT PRIMARY KEY`
  - 通过临时表+复制+重命名的方式确保数据完整性
