...
| 桌面 GUI | v0.1.6 | Tauri 桌面应用(`gui/package.json`,自 v0.0.1 起独立版本化) |
...
- **全自动对话记忆**:每轮对话结束后由 AI 自动提取值得记住的偏好与约定(零操作),注入后续对话提示词;支持"跨对话记忆"开关——开启全局共享,关闭按工作区独立记忆互不共享
- **可调工作区布局**:侧栏与问题面板支持拖拽调宽、折叠收起与双击重置,布局状态持久化
- **分析报告**:一键导出 HTML / Markdown 格式的项目分析报告
- **检查更新**:启动时静默检查新版本,顶部提示条通知,点"下载更新"才开始下载(可取消),完成后一键安装;设置页支持手动检查与更新源配置
...
├── reports/             # 导出的分析报告
├── logs/                # 运行日志
├── chat_memory.json     # 对话记忆(跨对话全局记忆,用户偏好与项目约定)
├── chat_memory_<hash>.json  # 关闭跨对话记忆时,按工作区隔离的独立记忆
...
- **修复"无法连接 GitHub（HTTP 301）"导致检查更新失败**:仓库已迁移至 `Mr.Sliy--AI_Agent`,旧地址 `whureww/Mr.Sliy` 被 GitHub 永久重定向(301),而 Node 原生 HTTPS 请求不自动跟随重定向,检查更新因此必然失败——更新源改为新仓库地址,且版本清单拉取现在会手动跟随 301/302/307/308 重定向(最多 5 跳),仓库再次迁移也不受影响。v0.1.5 及更早版本均内置旧地址,无法通过应用内更新自动获取本版,请从 GitHub Release 页手动下载一次

### 桌面 GUI v0.1.5
> 发布日期: 2026-09-14

- **修复分析模式发送按钮不随主题色**:按钮禁用态(输入为空/会话锁定时的默认状态)底色硬编码琥珀浅橙,切换任意主题都不变——现禁用底色与图标色改为由当前主题 accent 实时派生的半透明色,日/夜分态取不同透明度,随主题即时切换;主按钮禁用态文字色同步改为派生色(原先继承白色,浅底上几乎不可见)
- **更新下载改为手动触发**:顶部更新提示条不再自动开始后台下载,用户点"下载更新"才开始;下载中支持取消(终止请求并复位状态,修复取消后仍被换源逻辑当网络错误继续静默续传的问题);下载中也可直接关闭提示条
- **入库补齐 v0.1.4 功能的后端接口**:MCP 可用性自检(`GET /api/mcp/selftest`,并修复状态接口展示 `127.0.0.1` 而服务实际监听 `localhost` 的地址不一致)、缺陷统计接口补充未修复严重度分布与项目规模、扫描完成时补记项目文件数/代码行数(v0.1.4 安装包已含这些代码但未纳入版本库,本版一并入库)

### 桌面 GUI v0.1.4
> 发布日期: 2026-09-14

- **全自动记忆(零操作)**:每轮对话结束后异步由 LLM 判断本轮是否有值得长期记住的内容(偏好/约定/事实),有则自动写入记忆库——移除中文正则启发式提取与设置页手动添加入口,记忆列表仅用于查看与纠错(单条删除/清空);条目带"自动/手动"来源徽标,写入带双层去重与成本控制(短消息跳过、单飞互斥、单轮最多 3 条)
- **跨对话记忆开关**:设置页新增开关——开启(默认)所有对话共享全局记忆;关闭后记忆功能仍生效,但按工作区独立存储(`chat_memory_<hash>.json`)、互不共享,也不读取全局记忆;MCP 外部调用不受影响
- **修复换源产出损坏安装包**(os error 14001 "并行配置不正确"):下载器切换下载源时未截断 `.part` 文件,把上一源已写入的残页与新源数据缝合成损坏安装包(sha256 仅覆盖新源数据反而校验通过);现换源时销毁旧写流并重建截断,同时吞掉换源竞态的流错误防止进程崩溃
- **主工作区布局可调**:侧栏(180–400px)与编辑模式问题面板(260–560px)支持拖拽分隔条调宽、`«`/`»` 折叠收起、双击重置;宽度与折叠状态持久化,重启保留
- **分析会话输入区优化**:输入框与发送按钮加大、字号提升;占位文字与下方快捷提示去重;侧栏会话搜索框加大更好点击
- **质量评分改为加权缺陷密度**:原 `100 − 未处理缺陷数×2` 既无视严重度(1 个高危=1 个低危)、也无项目规模分母(大项目恒低分),且 `scan_project.total_lines` 从未填充恒为 0——现扫描完成时补记项目文件数与代码行数,评分 = 100 − (高危×10+中危×3+低危×1) ÷ 千行代码 × 15;行数未知的存量项目回退按加权计数扣减并提示"重新扫描后启用密度评分"
- **编辑模式 AI 对话改流式输出**:悬浮 AI 助手从一次性展示改为逐字输出(带闪烁光标),结束后才展示代码修改确认卡片与用量统计;中途停止保留已流出文本
- **MCP 可用性自检**:设置页 MCP 区块新增"检测可用性"按钮,真实走一遍 HTTP JSON-RPC 三步握手(initialize → tools/list → ping),失败逐步显示 ✓/✗ 与耗时;修复状态接口展示 `127.0.0.1` 地址但服务实际监听 `localhost`(部分 Windows 解析为 IPv6 ::1)导致外部客户端照抄配置连不上的问题
- **修复主题切换后部分控件不跟随**:按钮禁用态、停止按钮悬停、输入条聚焦光环、流水线步骤呼吸光环、右键菜单禁用/危险项、滚动条等 6 处硬编码亮色改为分态变量;补齐 `--success-text` 等状态文字色的亮色定义;聚焦环/呼吸光环颜色现从当前主题 accent 动态派生,任意主题色下都协调
- **编辑器多标签溢出收纳**:多开文件超出标签栏宽度时自动收起为右侧 `▼ n` 下拉面板,列出全部已开文件(含未保存圆点)供切换/关闭,不再被横向滚动截断;未溢出时保持平铺

### 桌面 GUI v0.1.3
> 发布日期: 2026-09-14

- **质量概览改为按项目查看**:原概览聚合了所有扫描过项目的历史缺陷(全局 19203 条缺陷时评分恒为 0),现顶部新增项目选择器,评分/修复率/严重度/语言分布仅反映所选项目的扫描情况,默认选中最近扫描的项目
- **修复同一项目反复扫描导致缺陷重复累计**:扫描时按路径(忽略大小写与尾斜杠)复用已有项目记录,历史重复记录的缺陷与任务自动并入并去重
- **修复项目列表接口 500**:`/api/projects` 使用了 better-sqlite3 禁用的双引号字符串字面量,接口自上线以来不可用
- **修复"检查更新"一键装回同版本**:更新目录残留的历史安装包被恢复扫描无条件恢复为"可安装"(不比对版本),导致 0.1.2 用户点检查更新后安装按钮指向同版本旧包——恢复扫描现按 GUI 上报的当前版本做门控,仅采纳更新的安装包

### 桌面 GUI v0.1.2
> 发布日期: 2026-09-12

- **修复覆盖安装后每次启动仍提示"发现新版本"**:启动恢复扫描到已下载的安装包时未对比版本号即弹横幅——现在仅当包版本 > 当前版本才提示,并携带正确的当前版本号
- **修复快捷方式显示未知图标**:安装脚本图标引用了从未打包的 `{app}\mrsliy.ico`,现改为直接使用 exe 内嵌图标
- 注:v0.1.1 已装用户若曾被同版本安装包反复提示,覆盖安装本版即可

### 桌面 GUI v0.1.1
> 发布日期: 2026-09-11

- **修复"一键安装"报 `%1 不是有效的 Win32 应用程序 (os error 193)`**:下载的安装包本身完好(sha256 校验通过、手动可运行),但应用内启动时偶发被系统层拒绝——典型诱因为杀软实时扫描锁住镜像读取、`canonicalize` 产生的 `\\?\` verbatim 路径在部分系统层的兼容性问题
- 启动器健壮化:启动前 PE 头校验(损坏直接报明确错误)、去除 `\\?\` 前缀、失败自动等待重试 + `cmd start`(ShellExecute 语义)三级兜底、全程写入 `~/.mr-sliy/logs/installer.log` 留痕
- ⚠️ 若 v0.1.0 曾安装失败,直接覆盖安装本版即可

### 桌面 GUI v0.1.0
> 发布日期: 2026-09-11

- **修复页面切换"过渡结束但内容未显示完全"**:过渡期原为固定 480ms 定时器,与页面实际数据加载脱钩——定时器一结束就解除点击屏蔽,但文件树/统计/MCP 状态等异步数据还在加载
- 过渡期改为**数据就绪驱动**:切换即显示顶部缓冲进度条并隐藏内容,各页面首屏数据加载完成后(`onReady` 回调)才淡入完整内容——加载结束=内容显示完全
- 四页面接入就绪协议:主工作区(工作区/会话恢复完成)、质量概览(统计数据落定)、设置(提供商/记忆/更新记录/MCP 状态全部落定)、优化对比(同步渲染挂载即就绪)
- 兜底 1.2s 强制就绪,个别接口异常时不会永久卡住交互
- 缓冲进度条改循环扫过动画,慢加载不再停格在末端

### 桌面 GUI v0.0.10
> 发布日期: 2026-09-11

- **修复快捷方式图标显示旧默认图**(大黄十字):覆盖安装时 exe 路径不变,Windows 图标缓存不失效,桌面/开始菜单沿用旧缓存——即使 exe 内嵌图标早已换成新 logo
- 快捷方式现显式指向 `{app}\mrsliy.ico`(路径随安装刷新,缓存必然失效);安装器/卸载入口图标同步更新
- 图标统一从 `gui/src/assets/logo.svg` 高清生成(`node scripts/gen-icons.js`,sharp 栅格化 + 16~256 七层 ICO),exe 内嵌图标、安装器图标、托盘图标同源
- 页面切换加载缓冲:淡入过渡 + 顶部进度条,过渡期短暂屏蔽点击防误触
- 设置页折叠全面丝滑化(grid-rows 过渡,高度自适应内容);更新记录默认展示最新 5 条,更早的自动折叠
- 精简设置页冗余文案(检查更新/更新记录/MCP/外观/语言五处)
